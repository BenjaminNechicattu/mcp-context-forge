...
def process_credentials(creds: Dict[str, Any], headers: Optional[Dict[str, str]] = None, query_params: Optional[Dict[str, str]] = None) -> tuple[Dict[str, str], Dict[str, str]]:
    ...
    auth_type = creds.get("security_scheme", None)
    details = creds.get("runtime_credentials", {})
...
async def get_runtime_credentials(connection_id: str, access_token: str, env: str = "draft") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                url,
                headers=headers,
                timeout=httpx.Timeout(CONNECT_TIMEOUT_SEC + READ_TIMEOUT_SEC, connect=CONNECT_TIMEOUT_SEC)
            )
            response.raise_for_status()
            return response.json()
    except httpx.HTTPStatusError as e:
        _logger.error(f"[WXO Connections]HTTP error getting credentials for connection_id {connection_id}: {e.response.status_code} - {e.response.text}")
        try:
            return e.response.json()
        except Exception:
            _logger.debug("[WXO Connections] Could not parse error response as JSON")
        return {}
    except Exception as e:
        _logger.error(f"[WXO Connections] Unexpected error getting credentials for connection_id {connection_id}: {e}")
        return {}
